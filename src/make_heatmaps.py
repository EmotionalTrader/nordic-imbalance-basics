...
def heatmap(df: pd.DataFrame, value_col: str, title: str, outpath: Path):
    pv = df.pivot_table(
        index="hour", columns="weekday", values=value_col, aggfunc="mean"
    )
    ...
    cb.ax.set_ylabel(value_col, rotation=270, labelpad=12)
    fig.tight_layout()
...
def main():
    ...
    heatmap(
        df,
        "price_eur_mwh",
        "Imbalance Price (€/MWh): Hour × Weekday",
        outdir / "heatmap_price.png",
    )
    heatmap(
        df,
        "imbalance_volume_mwh",
        "Imbalance Volume (MWh): Hour × Weekday",
        outdir / "heatmap_volume.png",
    )
