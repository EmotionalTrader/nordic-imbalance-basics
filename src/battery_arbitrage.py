...
def simulate(
    prices: pd.DataFrame, e_mwh: float, p_mw: float, eta_rt: float, soc0: float
) -> pd.DataFrame:
    """
...
def write_stats_md(
    ...
    md.append(
        f"**Params:** E={e_mwh} MWh  |  P={p_mw} MW  |  η={eta_rt:.2f}  |  SOC₀={soc0} MWh"
    )
    md.append("")
    ...
    md.append(
        f"- Energy in (grid→bat): {e_in:.1f} MWh  |  Energy out (bat→grid): {e_out:.1f} MWh"
    )
    md.append(
        f"- Approx cycles over period: {approx_cycles:.2f}  ( = total discharge MWh / E )"
    )
    md.append("")
    md.append(
...
def main():
    ap = argparse.ArgumentParser(
        description="Greedy battery arbitrage on day-ahead prices."
    )
    ...
    ap.add_argument(
        "--eta", type=float, default=0.90, help="Round-trip efficiency (0..1)"
    )
    ...
    schedule = simulate(
        prices, e_mwh=args.e_mwh, p_mw=args.p_mw, eta_rt=args.eta, soc0=args.soc0
    )

    # Outputs
    zone = args.zone or (
        df["area"].iloc[0] if "area" in df.columns and len(df) else "ZONE"
    )
    title = (
        args.title
        or f"{zone} battery-lite DA arbitrage (E={args.e_mwh}MWh, P={args.p_mw}MW, η={args.eta:.0%})"
    )
